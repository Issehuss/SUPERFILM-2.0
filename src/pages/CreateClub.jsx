...
function CreateClub() {
  ...
  // ─────────────── Premium club-limit guard ───────────────
  const [allowed, setAllowed] = useState(false);
  const [showGate, setShowGate] = useState(false);
  const { canCreateAnotherClub, limits } = useEntitlements();

  useEffect(() => {
    (async () => {
      const ok = await canCreateAnotherClub();
      setAllowed(ok);
      if (!ok) setShowGate(true);
    })();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center px-4">
      {/* Upgrade modal if blocked */}
      <UpgradeGateModal
        open={showGate}
        onClose={() => setShowGate(false)}
        limit={limits.limitForPlan || 1}
        premiumLimit={5}
      />

      {/* Only render wizard if allowed */}
      {allowed && (
          ...
          {step === 3 && (
            <>
              ...
              <p className="mb-4 text-zinc-400">
                Decide if your club requires a minimum number of members before going live.
              </p>
